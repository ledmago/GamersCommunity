	...
	var documentTemplate = '\
		<html>\
			<head>\
				...
				<style>body{font-family: "PT Sans Narrow", sans-serif;}</style>\
				</head>\
			<body id="iframeBody">\
				INSERT:CONTENT:END\
			</body>\
	...
	switch (this.action)
	{
		...
		case "link":
			...
			else
			{
				...
				var theURL = prompt("Url Gir:", "http://");
		...
		case "image":
			var theImage = prompt("Resim Urlsini Buraya Gir:", "");
			
			if (theImage != null && theImage != "")
			{
				var theAlt = prompt("Resim İçeriğini Buraya Gir :", "");
				...
				if (theIframe.contentWindow.document.selection)
				{
					...
					theRange.pasteHTML("<img  class='postresim2' src=\"" + theImage + "\" />");
