...
if (stristr($cekilen_veri, "profile_banner")){
	 ...
	 Yayın Şuan Açık / Stream is Online Now';
...
else
{
	...
	Yayın Kapalı / Stream is Offline Now';
}
?>
...
<?php	echo $duyuru;?></marquee></div><div id="postalani" style="overflow:hidden">								<iframe name="content" width="100%" height="100%" src="indexorta.php"></iframe>													</div><!-- POSTALANI BİTTİ -->			<div id="sagblock" style="overflow-x:scroll">			    <div id="sayac"><center>Yayın Sayacı</center><center><font id="yazdirburaya" size="13px" id="say">00:00 </font></center></div>				   <script type="text/javascript">function ojidanieNG()  {  var today = new Date();     var BigDay = new Date("December 4, 2014, 19:00"); // gelecekteki yılın tarihini buraya yazıyoruz.  var timeLeft = (BigDay.getTime() - today.getTime());     var e_daysLeft = timeLeft / 86400000;  var daysLeft = Math.floor(e_daysLeft); // Kalan gün sayısı     var e_hrsLeft = (e_daysLeft - daysLeft)*24;  var hrsLeft = Math.floor(e_hrsLeft); // Kalan saat sayısı     var e_minsLeft = (e_hrsLeft - hrsLeft)*60;  var minsLeft = Math.floor(e_minsLeft); // Kalan dakika sayısı     var seksLeft = Math.floor((e_minsLeft - minsLeft)*60); // Kalan saniye sayısı     if (BigDay.getTime() > today.getTime() )  document.getElementById("say").innerHTML = '<font color=red>Yeni Yıla</font>: '+daysLeft+' Gün, '+hrsLeft+' Saat, '+minsLeft+' dakika, '+seksLeft+' Saniye'  else  document.getElementById("say").innerHTML = '<font color=red>Hoşgeldin 2013</font>!!!'  }  setInterval("ojidanieNG()", 50)  </script>				<div id="bildirimler"><div id="baslikbild">Bildirimler</div>				<?php							mysql_query("SET NAMES UTF8");	$sorgu=mysql_query("SELECT * FROM `bildirimler` ORDER BY  id DESC");				  while($gonderi = mysql_fetch_array($sorgu))    {	$mesajcek = $gonderi["mesaj"];	echo '<div id="bild">'.$mesajcek.'</div>';		}				?>							</div>			    <div id="sinema" onClick="sagakaydir()"><img src="http://png-5.findicons.com/files/icons/1035/human_o2/32/emblem_camera.png" >&nbsp;Sinema Modu</div>								 <div id="kurallar"><a class="nolink" href="kurallar.php" target="content"><img src="http://png-1.findicons.com/files/icons/608/fight_club/32/rules_of_fight_club.png" >&nbsp;Kurallar</a></div>				 		<!--		 <div id="ekipmanlar" onclick="window.location = 'http://zeonnn.com/#section-ekipmanlarim'"><img src="http://png-4.findicons.com/files/icons/262/hydropro_hardware/32/hp_keyboard_2.png">&nbsp;Ekipmanlar</div>				 --><a class="nolink" href="akis.php" target="content"><div id="ekipmanlar">Yayın Akışı</a></div>				<!--  <div id="ekipmanlar" >İstek Dilek Kutusu</div>			    								<div id="kurallar" onclick="window.location= 'http://zeonnn.com/'">Zeonnn.com</div>	-->		    			</div><!-- SAG BLOCK -->			<div style="clear:both">        </div><!-- BİTİŞ CONTAİNER -->					<script>	function sagakaydir()	{	    	   document.getElementById("sinemamodu").style.right = "0px";	     document.getElementById("sinemamodu").style.display = "block";	   	}	function kapatf()	{	          var genislik = document.body.offsetWidth;var fark = document.body.offsetWidth - 303;document.getElementById("sinemamodu").style.width = fark + "px";document.getElementById("sinemamodu").style.right = -fark + "px";	}var genislik = document.body.offsetWidth;var fark = document.body.offsetWidth - 303;//document.getElementById("container").style.width= fark + "px";		</script>    	<!-- Renklendir Necoyu :D --><script>setInterval(function(){var sayi = Math.floor(Math.random() * 6);var r1 = "#E24B75";var r2 = "#F3C110";var r3 = "#57BEC7";var r4 = "#F3F7E3";var r5  = "#B5D045";if(sayi == 1){ $('#iclogo').css({'background-color':r1 });}else if(sayi == 2){ $('#iclogo').css({'background-color':r2 });}else if(sayi == 3){ $('#iclogo').css({'background-color':r3 });}else if(sayi == 4){ $('#iclogo').css({'background-color':r4 });}else if(sayi == 5){ $('#iclogo').css({'background-color':r5 });}}, 1000);</script><!-- Renklendir Necoyu --></body></html>