...
<body>
		...
		<div class="col-md-15">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<h1> Ceza Nedir ? </h1>
						<p> Ceza Kullanıcıyı Muteler (Susturur) Kullanıcı Ceza Aldığında Konu Açamaz </p>
						<div class="form-group">
									<label>Kullanıcının Email Adresi</label>
									<input name="emailal" class="form-control" style="max-width:300px;" placeholder="Kullanıcı Email Adresi">
								</div>
									
						<button type="submit" name="cezalandir" class="btn btn-primary">Ceza Ver</button>
						
						</form>
						
						
								<?php 
						if(isset($_POST["cezalandir"]))
						{
						$emailceza = $_POST["emailal"];
						mysql_query("UPDATE `uyeler2` SET `ceza`= '1' WHERE email = '$emailceza'");
						echo "<script>alert('Kullanıcı Cezalandırıldı')</script>";
						...
						<h1>Şuan Cezalı Kullanıcılar </h1>
						...
						<tbody><tr>
									<?php					
	$cekw = mysql_query("SELECT * FROM `uyeler2` WHERE ceza='1' ORDER BY id DESC");
	...
	echo "
						...
						<tr>
						<td><a href='cezasil.php?id=$idceza'>Cezayı Kaldır</a></td>
						<td>Şuanda Cezalı</td>
								<td>$isimal</td>
										<td>$emailal</td>
